...
class AJAX_Hits_Counter {
	...
	public function init() {
		if ( is_admin() ) {
			...
			register_importer(
				__CLASS__ . '_Importer',
				$this->plugin_title . ': ' . __( 'Import from', 'ela' ) . ' WP-PostViews',
				__( 'Imports views count (hits) from plugin', 'ela' ) . ' <a href="http://wordpress.org/plugins/wp-postviews">WP-PostViews</a> ' . __( 'to hits of', 'ela' ) . ' <a href="http://wordpress.org/plugins/' . $this->plugin_alias . '/">' . $this->plugin_title . '</a>.',
	...
	public function adminMenu() {
		...
		add_theme_page( 'Views counter settings', 'Views counter settings', 'administrator', 'ela-hitscount', array( $this, 'adminSettingsPage' ) );
	...
	public function adminSettingsPage() {
		...
		echo(
			'<div class="wrap">' .
			'<h2>' . $this->plugin_title . ': ' . __( 'Settings', 'ela' ) . '</h2>' .
		...
		echo(
			...
			'<label for="ajaxhc_exclude_recurred_views">&nbsp;' . __( 'Don\'t Count Repeated Views.', 'ela' ) . '</label>' .
			'</td>' .
			'</tr>' .
			'<tr valign="top">' .
			'<td colspan="2">' .
			...
			'<label for="ajaxhc_dont_count_admins">&nbsp;' . __( 'Don\'t count hits of admin users', 'ela' ) . '</label>' .
			'</td>' .
			'</tr>' .
			'</table>'
		);
	...
	public function adminAddMetaBoxPrint( $post, $metabox ) {
		...
		echo(
			'<label for="hits">' . __( 'Views count', 'ela' ) . '</label>&nbsp;&nbsp;' .
	...
	public function adminImporter() {
		...
		$html =
			'<div class="wrap">' .
			'<h2>' . $this->plugin_title . ': ' . __( 'Import from', 'ela' ) . ' WP-PostViews</h2>' .
			'<div class="clear"></div>';
		...
		if ( ! isset( $_POST['start'] ) || empty( $_POST['start'] ) ) {
			...
			if ( empty( $total ) ) {
				$html .=
					'<p>' . __( 'We found', 'ela' ) . ' <strong>' . __( 'no items', 'ela' ) . '</strong> ' . __( 'to import from WP-PostViews plugin', 'ela' ) . '.</p>' .
					'<p>' . __( 'Have I hice day', 'ela' ) . ' ;-)</p>';
			} else {
				$html .=
					'<p>' . __( 'We found', 'ela' ) . ' <strong>' . $total . ' ' . __( 'items', 'ela' ) . '</strong> ' . __( 'to import from WP-PostViews plugin', 'ela' ) . '.</p>' .
					'<p>' . __( 'To start import please click "Start import" button.', 'ela' ) . '</p>' .
					'<form method="post">' .
					wp_nonce_field( plugin_basename( __FILE__ ), 'ajax_hits_counter_nonce', true, false ) .
					'<input type="submit" value="' . __( 'Start import', 'ela' ) . '" class="button" name="start" />' .
					'</form>';
			}
			...
			if ( ! empty( $results ) ) {
				...
				$html .=
					'<p>' . __( 'Imported', 'ela' ) . ' <strong>' . $status['total'] . ' ' . __( 'items', 'ela' ) . '</strong> ' .
					'(' . __( 'inserted', 'ela' ) . ': <strong>' . $status['inserted'] . '</strong>, ' . __( 'updated', 'ela' ) . ': <strong>' . $status['updated'] . '</strong>, ' . __( 'skipped', 'ela' ) . ': <strong>' . $status['skipped'] . '</strong>)</p>' .
					'<p>' . __( 'Thank you for choosing our plugin.', 'ela' ) . '</p>';
			} else {
				$html .=
					'<p>' . __( 'We found', 'ela' ) . ' <strong>' . __( 'no items', 'ela' ) . '</strong> ' . __( 'to import from WP-PostViews plugin', 'ela' ) . '.</p>' .
					'<p>' . __( 'Have I hice day', 'ela' ) . ' ;-)</p>';
