            ...
            <div class="col12">
				...
				while ( have_posts() ) : the_post();
					...
					if ( class_exists( 'CSF' ) ) {
						...
						$stories_cdn_url = $opt['stories_cdn_url'];
					}
					$story_bg_image = get_the_post_thumbnail_url( get_the_ID(), 'ela_story_image', array( 'class' => 'img-responsive' ) );
					if ( $stories_cdn_url ) {
						// cdn url
						$story_bg_image = str_replace( home_url(), $stories_cdn_url, $story_bg_image );
					}
					?>
                    <div class='story_single_row row'>
                        ...
                        <div class="col5 alia_story_comments">
							...
							$output .= '<span class="story_item_author_avatar"><a class="meta_author_avatar_url" href="' . esc_url( get_author_posts_url( $author_id ) ) . '">'
							           . ela_filter_lazyload_images( get_avatar( $author_id, 40 ) ) . '</a></span>';
					...
					if ( ! empty( $prev_post ) ) {
						$story_bg_image = get_the_post_thumbnail_url( $prev_post->ID, 'ela_thumbnail_avatar', array( 'class' => 'img-responsive' ) );
						if ( $stories_cdn_url ) {
							// cdn url
							$story_bg_image = str_replace( home_url(), $stories_cdn_url, $story_bg_image );
						}
					...
					if ( ! empty( $next_post ) ) {
						$story_bg_image = get_the_post_thumbnail_url( $next_post->ID, 'ela_thumbnail_avatar', array( 'class' => 'img-responsive' ) );
						if ( $stories_cdn_url ) {
							// cdn url
							$story_bg_image = str_replace( home_url(), $stories_cdn_url, $story_bg_image );
						}
