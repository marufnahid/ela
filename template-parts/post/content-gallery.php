...
<article id="post-<?php the_ID(); ?>" <?php post_class( $post_classes ); ?>>
	...
	if ( ! is_single( get_the_ID() ) ) {
		...
		if ( has_post_thumbnail() && ( "wide" != $ela_content_width || $total_gallery_items < 2 ) ):
			...
			if ( isset( $ela_content_width ) && $ela_content_width == 'grid3col' ) {
			...
			if ( is_array( get_post_gallery( '', false ) ) && array_key_exists( 'ids', get_post_gallery( '', false ) ) ) {
				...
				if ( count( $gallery_images_ids ) > 1 ) {
					...
					foreach ( $gallery_images_ids as $id ) {
						...
						$src       = 'src=';
						if ( $lazyload_image_banner == 1 ) {
							$src = 'class="lazyload" src="data:image/gif;base64,R0lGODlhAQABAAAAACH5BAEKAAEALAAAAAABAAEAAAICTAEAOw==" data-src=';
						}

						if ( isset( $ela_content_width ) && "wide" == $ela_content_width && $num > 3 ) {
							break;
						} elseif ( ( isset( $ela_content_width ) && "grid" == $ela_content_width )
						           || ( isset( $ela_content_width ) && "two_coloumns_list" == $ela_content_width )
						           || ( isset( $ela_content_width ) && "grid3col" == $ela_content_width )
						) {
							if ( count( $gallery_images_ids ) == 3 && $num == 3 ) {
								...
								echo '<div class="gallery_single_image col item' . esc_attr( $num ) . '">
														<img ' . $src . '"' . wp_get_attachment_image_src( $id, 'ela_wide_thumbnail' )[0] . '" alt="' . esc_attr( $image_alt ) . '" />
													</div>';
								break;
							} elseif ( $num > 4 ) {
								break;
						...
						echo '<div class="gallery_single_image col item' . esc_attr( $num ) . '">
												<img ' . $src . '"' . wp_get_attachment_image_src( $id, 'ela_large_thumbnail' )[0] . '" alt="' . esc_attr( $image_alt ) . '" />
											</div>';
				...
				echo '<figure class="post_banner">';
				echo ela_filter_lazyload_images( get_post_gallery( get_the_ID(), false ) );
				echo '</figure>';
			};
		endif;
	};
    ...
    <div class="post_body <?php echo esc_attr( $post_banner_class ); ?>">
        ...
        <div class="post_info_wrapper">
			<?php if ( is_single( get_the_ID() ) ||
				...
				if ( ( $ela_content_width == 'two_coloumns_list' || $ela_content_width == 'grid' || $ela_content_width == 'grid3col' ) && ( $num < 3 || has_post_thumbnail() ) ) {
					echo ela_excerpt( 20 );
				} else {
					echo ela_excerpt( 40 );
				}
